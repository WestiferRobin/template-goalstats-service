...
from smoke import redis, request, smoke_stack, sql
from workflow import ROOT, Stack, install_signals, run, setup, tests, wait_ready
...
def smoke():
    ...
    print("Built DEV smoke and cleanup: PASS")
...
def certify():
    baseline = inventory()
    print("Docker baseline:", json.dumps({k: sorted(v) for k, v in baseline.items()}), flush=True)
    ...
    try:
        ...
        failed_smoke = fresh("dev")
        try:
            failed_smoke.migrate()
            failed_smoke.start()
            try:
                smoke_stack(failed_smoke, fault=True)
            except AssertionError:
                pass
            else:
                raise AssertionError("Expected smoke failure")
        finally:
            failed_smoke.stop(volumes=True)
            assert_absent(failed_smoke.project)
